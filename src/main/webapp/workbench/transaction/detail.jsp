<%@ page import="com.js.settings.domain.DicValue" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.Set" %>
<%@ page import="com.js.workbench.domain.Tran" %>
...
<head>
    ...
    <script type="text/javascript">
        ...
        function  changIcon(stage,indexi){
            //当前阶段
            ...
            if (currentPossibility==0){//当前阶段为0,后面两个一个是红叉一个是黑叉

                //遍历前7个
                for (var i=0;i<point;i++){
                //    ------------黑圈
                    $("#"+i).removeClass();//移出原来的样式
                    $("#"+i).addClass("glyphicon glyphicon-record mystage");//添加新样式
                    $("#"+i).css("color","#000000");//添加颜色
                }
                ...
                for (var i = point; i < <%=dvList.size()%>; i++) {
                    ...
                    //    红叉
                        $("#"+i).removeClass();//移出原来的样式
                        $("#"+i).addClass("glyphicon glyphicon-remove mystage");//添加新样式
                        $("#"+i).css("color","#FF0000");//添加颜色
                    }else {
                    //    黑叉
                        $("#"+i).removeClass();//移出原来的样式
                        $("#"+i).addClass("glyphicon glyphicon-remove mystage");//添加新样式
            ...
            }else {//当前阶段不为0,前7个绿圈,绿色标记,黑圈,后面两个一定是黑叉
                //遍历前7个
                for (var i = 0; i < point; i++) {
                        if (i==index){//如果是当前阶段
                        //绿色标记
                            $("#"+i).removeClass();//移出原来的样式
                            $("#"+i).addClass("glyphicon glyphicon-map-marker mystage");//添加新样式
                            $("#"+i).css("color","#90F790");//添加颜色
                        }else if (i<index){//如果小于当前阶段
                        //绿圈
                            $("#"+i).removeClass();//移出原来的样式
                            $("#"+i).addClass("glyphicon glyphicon-ok-circle mystage");//添加新样式
                            $("#"+i).css("color","#90F790");//添加颜色
                        }else {//如果大于当前阶段
                        //黑圈
                            $("#"+i).removeClass();//移出原来的样式
...
//                获取该阶段对应的可能性
                String listPosibility=map.get(listStage);

//                 如果遍历处理的阶段为0,就表示为后两个,红叉与黑叉
                if ("0".equals(listPosibility)){
//                    如果是当前的阶段,当前阶段就为红叉
...
//                      红叉
    ...
    <%
        ...
        }else {//如果当前节点的可能性不为0,前7个有可能为绿圈,绿色标记,黑圈,后面两个一定是黑叉
//            准备当前阶段的下标
            int index=0;
            for (int i = 0; i < dvList.size(); i++) {
                DicValue dv=dvList.get(i);
                String stage=dv.getValue();
//                String possibility=map.get(stage);
                if (stage.equals(currentStage)){//如果遍历出来的阶段是当前阶段,index就记录下标
                    index=i;
                    break;
                }
            }
            ...
            for (int i = 0; i < dvList.size(); i++) {
                ...
                if ("0".equals(listPosibility)){//如果遍历出来的阶段为0,说明就是后两个阶段,两黑叉
    %>
                <span id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')"
                      class="glyphicon glyphicon-remove mystage"
                      data-toggle="popover" data-placement="bottom"
